...
// HealthyAssetsScreen
// Purpose: Dedicated ERP-style administrative screen that lists only assets
// with status === 'Healthy'. Implements header, compact summary bar and a
// horizontally-scrollable data table suitable for administrative consoles.
...
export default function HealthyAssetsScreen({ navigation }: any) {
  ...
  const loadAssets = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch only assets with status=Healthy from backend
      const result = await apiRequest<{ assets: any[] }>(`/api/assets?status=Healthy`, { method: 'GET' });
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* ERP header: back button, title and subtitle, asset count */}
      <View style={styles.header}>
        ...
        <View style={styles.headerText}>
          ...
          <Text style={styles.subtitle}>Operational assets currently active in the system</Text>
        </View>
      ...
      {/* Summary bar: compact ERP-style metrics (kept small) */}
      <View style={styles.summaryBar}>
        ...
        <View style={styles.summaryItem}>
          <Text style={styles.summaryNumber}>{/* scannedToday placeholder */}—</Text>
      ...
      >
        {loading ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyText}>No assets with status "Healthy" were found.</Text>
          </View>
        ) : (
