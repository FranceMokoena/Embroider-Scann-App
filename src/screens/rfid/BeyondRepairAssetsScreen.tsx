...
// Purpose: Administrative screen that lists assets with status === 'Beyond Repair'.
...
export default function BeyondRepairAssetsScreen({ navigation }: any) {
  ...
  const loadAssets = useCallback(async () => {
    setLoading(true);
    try {
      const result = await apiRequest<{ assets: any[] }>(`/api/assets?status=Beyond%20Repair`, { method: 'GET' });
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerText}>
          <Text style={styles.title}>Beyond Repair</Text>
          <Text style={styles.subtitle}>Assets marked for replacement or write-off</Text>
        </View>
      ...
      >
        {loading ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyText}>No assets with status "Beyond Repair" were found.</Text>
          </View>
        ) : (
